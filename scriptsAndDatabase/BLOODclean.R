...
blood <- scan("rawTexts/cormac-mccarthy-blood-meridian.txt",what="character",sep="\n")
...
blood.para.matrix <- cbind(blood.title, blood.para.type, blood.para.id, blood.paragraphs)
...
blood.title <- rep("bloodMeridian", 7568)
blood.sents.type <- rep("sentence", 7568)
blood.sents.counter<-seq(1, 7568)
blood.sents.id <- paste0("BLOOD_MERIDIAN_", "SENT_", blood.sents.counter)
print(length(blood.sents.id))

blood.sents.matrix <- cbind(blood.title, blood.sents.type, blood.sents.id, blood.sents)
...
blood.title <- rep("bloodMeridian", 115182)
blood.words.type <- rep("word", 115182)
blood.words.counter <- seq(1, 115182)
blood.words.id <- paste0("BLOOD_MERIDIAN_", "WORD_", blood.words.counter)

blood.words.matrix <- cbind(blood.title, blood.words.type, blood.words.id, blood.words)

blood.words.df <- as.data.frame(blood.words.matrix, stringsAsFactors = FALSE)

con <- dbConnect(RSQLite::SQLite(), ":memory:", dbname="textTable.sqlite")
colnames(blood.words.df) <- c("Title", "Type", "ID", "Unit")
