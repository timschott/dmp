...
dorian <- scan("rawTexts/lyrical/oscar-wilde-picture-of-dorian-gray.txt",what="character", sep="\n")
...
for(i in seq(1:length(dorian.sents))){
  #if the sentence ends with a punctuation mark and the next character is a lowercase, combine them
  test <- substr(dorian.sents[i], nchar(dorian.sents[i])-1, nchar(dorian.sents[i]))
  test2 <- substr(dorian.sents[i+1], 1, 1)
  if(test2 %in% c(LETTERS, letters)){
...
bad_spots <- bad_spots[-c(1)]
dorian.sents[bad_spots]
dorian.sents <- dorian.sents[-c(bad_spots)]
# dorian.sents <- dorian.sents[-c(452)]
dorian.sents <- gsub("_", "", perl = TRUE, dorian.sents)
length(dorian.sents)
dorian.title <- rep("thePictureOfDorianGray", 6251)
dorian.sents.type <- rep("sentence", 6251)
dorian.sents.counter<-seq(1, 6251)
dorian.sents.id <- paste0("THE_PICTURE_OF_DORIAN_GRAY_", "SENT_", dorian.sents.counter)
dorian.label <- rep("1", 6251)
print(length(dorian.sents.id))
dorian.sents.matrix <- cbind(dorian.title, dorian.sents.type, dorian.sents.id, dorian.sents, dorian.label)
dorian.sents.df <- as.data.frame(dorian.sents.matrix, stringsAsFactors = FALSE)
colnames(dorian.sents.df) <- stock
con <- dbConnect(RSQLite::SQLite(), ":memory:", dbname="textTable.sqlite")
# dbExecute(con, "DELETE FROM textTable WHERE Type='sentence' AND Title='thePictureOfDorianGray'")

dbWriteTable(con, "textTable", dorian.sents.df, append=TRUE, row.names=FALSE)
dbGetQuery(con, "SELECT Unit FROM textTable WHERE Type='sentence' AND Title='thePictureOfDorianGray' LIMIT 2")
dbDisconnect(con)
...
dorian.words<- dorian.words[which(dorian.words!="^'")]
dorian.words<- dorian.words[which(dorian.words!="'")]
print(length(dorian.words))

dorian.title <- rep("thePictureOfDorianGray", 79289)
dorian.words.type <- rep("word", 79289)
dorian.words.counter <- seq(1, 79289)
dorian.words.id <- paste0("THE_PICTURE_OF_DORIAN_GRAY_", "WORD_", dorian.words.counter)
dorian.label<- rep("1", 79289)
dorian.words.matrix <- cbind(dorian.title, dorian.words.type, dorian.words.id, dorian.words, dorian.label)

dorian.words.df <- as.data.frame(dorian.words.matrix, stringsAsFactors = FALSE)

con <- dbConnect(RSQLite::SQLite(), ":memory:", dbname="textTable.sqlite")
colnames(dorian.words.df) <- c("Title", "Type", "ID", "Unit", "Label")
# dbExecute(con, "DELETE FROM textTable WHERE Type='word' AND Title='dorian'")
dbWriteTable(con, "textTable", dorian.words.df, append=TRUE, row.names=FALSE)
dbGetQuery(con, "SELECT * FROM textTable WHERE Type= 'word' AND Title='thePictureOfDorianGray' LIMIT 10")
dbDisconnect(con)
...
dorian.title <- rep("thePictureOfDorianGray", 1524)
dorian.para.type <- rep("paragraph", 1524)
dorian.para.counter<-seq(1, 1524)
dorian.para.id <- paste0("THE_PICTURE_OF_DORIAN_GRAY_", "PARAGRAPH_", dorian.para.counter)
dorian.label <- rep("1", 1524)
print(length(dorian.para.id))
dorian.para.matrix <- cbind(dorian.title, dorian.para.type, dorian.para.id, dorian.paragraphs, dorian.label)
...
dbGetQuery(con, "SELECT Unit FROM textTable WHERE Type='paragraph' AND Title='thePictureOfDorianGray' LIMIT 2")
dbDisconnect(con)
